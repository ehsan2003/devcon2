    ...
    protected middleware: CommentsInsertAuthorizedRequestHandler[]
        ...
        (async (req, res) => {
            ...
            if (reqBody.responseTo) {
                ...
                if (!parent)
                    throw new NotFoundError(ErrorCodes.ERROR_COMMENTS_INSERT_AUTHORIZED_$_PARENT_NOT_FOUND, 'responseTo not found');
                if (!reqBody.post.equals(parent.forPost))
                    throw new ConflictError(ErrorCodes.ERROR_COMMENTS_INSERT_AUTHORIZED_$_POST_CONFLICT, 'response post is not equal to parent post');
            }
