...
class Insert extends BaseController<TagsInsertRequestHandler> {
    ...
    protected middleware: TagsInsertRequestHandler[]
        ...
        (async (req, res) => {
            ...
            const tag = await Tag.updateOne({_id: id || new Types.ObjectId()}, {slug}, {upsert: true}).catch(this.handleUniqueError(Codes.TAGS_INSERT_$_DUPLICATE_SLUG, 'duplicate slug'));
