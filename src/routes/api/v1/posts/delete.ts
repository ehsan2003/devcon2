...
class Delete extends BaseController<PostsDeleteRequestHandler> {
    ...
    protected middleware: PostsDeleteRequestHandler[]
        ...
        if (!post)
            throw new NotFoundError(Codes.POST_DELETE_$_NOT_FOUND, 'post not found');
        if (post.visible && user.role === Roles.contributor)
            throw new AccessForbiddenError(Codes.POST_DELETE_$_ACCESS_FORBIDDEN, 'deleting this post is not permitted by you');
        req.data = post;
        next();
    }
        , async (req, res) => {
